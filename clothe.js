document.addEventListener("DOMContentLoaded", () => {
    ...
    let category = "clothes"; // Default fallback
    
    if (pathName.includes("footwear") || pathName.includes("shoes")) {
        category = "footwear";
    } else if (pathName.includes("saree") || pathName.includes("ethnic")) {
        category = "saree";
    } else if (pathName.includes("electronic") || pathName.includes("gadget")) {
        category = "electronics";
    } else if (pathName.includes("bag") || pathName.includes("luggage")) {
        category = "bags";
    }
    ...
    const categoryConfigs = {
        ...
        saree: {
            subTypes: ["Kanjivaram Silk", "Banarasi Brocade", "Chiffon Print", "Cotton Handloom", "Organza Floral"],
            titles: ["Sukhmani Saree", "Vardhman Silk", "Meera Fashion Saree", "Kothari Collection", "Sanskriti Designer"],
            prefix: "Saree"
        },
        electronics: {
            subTypes: ["Bluetooth 5.3", "Noise Cancelling", "Fast Charging", "AMOLED Display", "Ultra HD"],
            titles: ["Pro Wireless Earbuds", "Smart Fitness Watch", "Power Bank 20000mAh", "Bass Soundbar", "HD Action Cam"],
            prefix: "Tech"
        },
        bags: {
            subTypes: ["Genuine Leather", "Waterproof Nylon", "Canvas Travel", "Vegan Leather", "Padded Polyester"],
            titles: ["Urban Travel Backpack", "Classic Leather Tote", "Executive Laptop Bag", "Mini Crossbody Bag", "Duffle Gym Bag"],
            prefix: "Bag"
        }
    };

    const activeConfig = categoryConfigs[category] || categoryConfigs.clothes;

    // 3. 50 Dynamic Items Generator
    const dynamicProductList = Array.from({ length: 50 }, (_, i) => {
        const id = (category === "clothes" ? 100 : category === "footwear" ? 200 : 300) + i + 1;
        ...
        return {
            ...
            fabricTag: `${activeConfig.subTypes[subTypeIndex]} Fabric`,
    ...
    // 4. Render Engine (Strictly Matches Your CSS Classes)
    function renderProducts(items) {
        if (!cpgContainer) return;
        cpgContainer.innerHTML = "";

        if (items.length === 0) {
            cpgContainer.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 3rem; font-weight: 700; color: var(--color-primary);">No products found matching your search.</div>`;
            return;
        }
    ...
    // 5. Unified Search Handler (Supports both ID '#search' and '#cpgGlobalSearch')
    ...
    // Clear Search Input Button Feature
...
// Cart Badge Management System (Supports '#cartCount' and '#cpgCartCounter')
